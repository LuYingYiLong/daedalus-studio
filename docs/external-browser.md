...
AI 连接目标标签页后显示持续的 AI 光标；只读查看时先停靠右上角，读取、等待模型回复及工具调用间隔不会使它自动消失。实际点击或填写后，光标移至对应控件并显示高亮；滚动显示短暂方向提示。等待时不伪造移动或点击，系统鼠标不受影响。

切到其他标签页时隐藏，返回目标页只显示当前状态、不重播操作。模型截图会临时隐藏光标，截图完成后恢复。当前 LLM 轮次结束（包括发布方案等待用户确认）、取消、退出控制或断连时清除；扩展心跳失效时也会清理残影，迟到心跳不能恢复已结束轮次的光标。

更新此逻辑需同时重启 Studio 并在扩展管理页重新加载扩展，以便新的心跳与页面反馈代码一起生效。
