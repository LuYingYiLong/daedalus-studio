...
1. 使用匹配的 Backend 源码构建。启动 Windows x64 Studio，在“设置 → 桌面感知”中开启“允许 AI 请求观察窗口”（默认关闭）。
...
4. “设置 → 桌面感知”集中提供权限开关和本地诊断；诊断区需要先在常规设置开启开发者模式。本地诊断不授权 AI、不添加附件、不调用模型，也不持久化诊断内容，离开页面或关闭设置窗口会清理结果。旧的感知 Dock 项会被移除，其他面板及尺寸保留。
