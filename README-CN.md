...
<p align="center">
  面向软件项目的桌面 AI 开发工作台：理解项目、执行工具、修改文件，并验证实际结果。
</p>
...
Daedalus Studio 是一套面向软件项目的桌面 AI 开发环境。它把持久化会话、可审查的工具调用、文件与 Git 差异、终端验证、MCP 集成以及受管本地后端整合进一个原生桌面应用。

它面向的是能够留下真实、可追溯项目改动的开发任务，而不只是一次聊天。

## 主要特点

- **理解项目工作区**：识别工作区路径、文件、资源、脚本、项目设置和项目依赖。
...
- **受管组件**：由 Studio 校验、安装、更新、修复和回滚 Daedalus Backend 与可选编辑器集成。

Studio 是桌面客户端与生命周期管理者；Backend 负责执行与持久化；可选编辑器集成可以提供编辑器上下文和桥接能力。每个 Studio 版本都会固定并检查相关组件版本，避免不兼容组件被静默混用。
...
- 检查与编辑项目文件、资源、脚本、设置和依赖。
- 在集成声明对应能力时，使用带预检、fingerprint 和可逆事务的类型化编辑器桥接 Patch。
- 在支持的情况下使用编辑器上下文、导航、预览、诊断和只读调试能力。
- 编辑器集成是可选的；核心工作区、终端、Git 和 MCP 功能无需编辑器即可使用。
...
Daedalus Backend 内置了覆盖项目分析、文件操作、设置、诊断、只读调试、编辑器上下文和运行时控制的一组 MCP 工具。
...
4. 完成新手引导。
5. 添加**工作区**。如果使用可选编辑器集成，请在对应的设置页面中完成配置。
...
提交问题时，请提供 Studio 版本、Backend 版本、已启用集成的版本、复现步骤和脱敏后的启动或会话诊断。请勿提交 API Key 或自定义 MCP Secret。
...
- [Daedalus Backend](https://github.com/LuYingYiLong/daedalus-backend)：运行时、会话、供应商、Workflow、工具、MCP 与可选编辑器集成。
- [Godot Daedalus](https://github.com/LuYingYiLong/godot-daedalus)：可选的 Godot 编辑器集成与 Editor Bridge。
