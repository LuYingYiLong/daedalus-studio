...
int wmain(int argc, wchar_t **argv) {
  try {
    ...
    for (;;) {
      uint32_t size = 0;
      if (!std::cin.read(reinterpret_cast<char *>(&size), 4))
        break;
      if (!size || size > 16384)
        return 3;
      std::string input(size, '\0');
      if (!std::cin.read(input.data(), size))
        return 3;
      Json response;
      text(response, L"id", "invalid");
      number(response, L"version", 1);
      try {
        ...
        if (id.empty() || id.size() > 160 ||
            request.GetNamedNumber(L"version") != 1)
        ...
        if (method == L"select") {
          ...
          exact(params, {});
          if (method == L"hello") {
            number(result, L"version", 1);
            jsonBoolean(result, L"readOnly", true);
