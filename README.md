...
The Daedalus backend ships with 85 Godot MCP tools covering offline project analysis, file and scene editing, project settings, LSP diagnostics, read-only DAP debugging, editor state inspection, and runtime control.
...
When `daedalus-bridge` is not present, `npm run dev` first reuses a verified package in `build/daedalus-bridge`. If no cache is available, it tries the fixed Bridge release. A network or certificate failure no longer prevents the Studio development server from starting; Godot Bridge features remain unavailable until a package is prepared. The error is still fatal for `npm run build:win`, `npm run pack:win`, and other production packaging commands.
...
Use one of these fixes, in order:
...
4. If Bridge is not needed, run `npm run dev` again. Development mode will continue without Bridge packaging and show the missing package in the Godot Projects page; this does not affect ordinary chat development.
