...
<p align="center">
  A desktop AI workbench for software projects with an agent that can inspect, edit, run tools, and verify its work.
</p>
...
- **Project-aware workspaces** — understand workspace paths, files, resources, scripts, project settings, and project dependencies.
...
- **Managed integrations** — Studio verifies, installs, updates, repairs, and rolls back the Daedalus backend and optional editor integrations.

Studio is the desktop client and lifecycle owner. The backend is the execution and persistence layer. Optional editor integrations can provide editor context and bridge capabilities. Their versions are pinned in each Studio release and checked before startup, so incompatible components are not silently mixed.
...
### Project and editor integrations

- Inspect and edit project files, resources, scripts, settings, and dependencies.
- Use typed editor bridge patches with preflight checks, fingerprints, and reversible transactions when an integration advertises those capabilities.
- Work with editor context, navigation, previews, diagnostics, and read-only debugging when supported by the connected integration.
- Keep editor-specific tools optional; the core workspace, terminal, Git, and MCP features work without an editor.
...
The Daedalus backend ships with a broad MCP tool set for project analysis, file operations, settings, diagnostics, read-only debugging, editor context, and runtime control.
...
4. Finish the first-run onboarding guide.
5. Add a workspace. If you use an optional editor integration, configure it from the corresponding settings page.
...
- Workspace paths are validated before file, Git, editor, and terminal operations.
...
Development setup, backend and Bridge layout, startup troubleshooting, validation, and packaging instructions are maintained in [CONTRIBUTING.md](CONTRIBUTING.md).
...
Bug reports should include the Studio version, backend version, any enabled integration versions, reproduction steps, and sanitized startup or session diagnostics. Do not include API keys or custom MCP secrets.
...
- [Daedalus Backend](https://github.com/LuYingYiLong/daedalus-backend) — runtime, sessions, providers, workflows, tools, MCP, and optional editor integrations.
- [Godot Daedalus](https://github.com/LuYingYiLong/godot-daedalus) — optional Godot editor integration and Editor Bridge.
