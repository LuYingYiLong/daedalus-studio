...
import { Alert, Badge, Button, Dropdown, Input, message, Modal, Spin, Tooltip, Tree, Typography } from "antd";
...
function createSessionTreePresentation(
	...
	const isDeletingWorktree: boolean = options.deletingWorktreeSessionId === session.id;
				...
				: [
						{
							...
							icon: isDeletingWorktree ? <Spin size="small" /> : <Icon name="remove" />,
							danger: true,
							disabled: options.deletingWorktreeSessionId !== null || isRunning,
						},
					]),
			{
				key: "open",
					...
					<span className={styles.sessionMenuItem}>
						...
						<span
							className={`${styles.sessionEndSlot} ${
								isRunning || isMoving ? styles.sessionEndSlotRunning : ""
							}`}
						>
...
function createSessionTreeNode(
	...
	return {
		...
		className: sectionKey === "projects" ? styles.projectSessionTreeNode : undefined,
		kind: "session",
		sectionKey,
		workspaceId,
...
function createProjectTreeData(
	...
	return workspaces.map((workspace: WorkspaceConfig): ProjectTreeNode => {
		...
		return {
			...
			className: styles.workspaceTreeNode,
			title: (
				<Dropdown menu={actionMenu} trigger={["contextMenu"]}>
					<span
						...
						aria-label={
							options.draggingSessionId === null
								? undefined
								: labels.moveSessionToWorkspaceAria(workspace.name)
						}
						onDragEnter={(event: DragEvent<HTMLSpanElement>): void => {
							const sessionId: string =
								event.dataTransfer.getData("application/x-daedalus-session-id") ||
								options.draggingSessionId ||
								"";
							if (sessionId.length === 0 || !options.canDropSessionOnWorkspace(sessionId, workspace))
								return;
							event.preventDefault();
...
function WorkspaceTree({
	...
	const [expandedSectionKeys, setExpandedSectionKeys] = useState<WorkspaceTreeSectionKey[]>(() => {
		return reconcileWorkspaceTreeOrder(
			initialWorkspaceTreeOrder,
			initialWorkspaces,
			filterVisibleSessions(initialSessions),
	...
	const [permanentWorktreeTarget, setPermanentWorktreeTarget] = useState<WorkspaceConfig | null>(null);
	const [permanentWorktreeName, setPermanentWorktreeName] = useState<string>("");
	const [permanentWorktreeSources, setPermanentWorktreeSources] = useState<Record<string, WorktreeSourceOptions>>({});
	const [isCreatingPermanentWorktree, setIsCreatingPermanentWorktree] = useState<boolean>(false);
	const [deletingWorkspaceId, setDeletingWorkspaceId] = useState<string | null>(null);
	const [renameTargetSession, setRenameTargetSession] = useState<SessionMetadata | null>(null);
	const [deleteWorktreeTarget, setDeleteWorktreeTarget] = useState<SessionMetadata | null>(null);
	const [deletingWorktreeSessionId, setDeletingWorktreeSessionId] = useState<string | null>(null);
	...
	const expandedSectionKeysRef = useRef<WorkspaceTreeSectionKey[]>(expandedSectionKeys);
	...
	const labels: WorkspaceTreeLabels = useMemo((): WorkspaceTreeLabels => {
		return {
			...
			pinSessionAria: (sessionTitle: string, pinned: boolean): string =>
				t(pinned ? "workspaceTree.aria.unpinSession" : "workspaceTree.aria.pinSession", { sessionTitle }),
			newSessionInWorkspaceAria: (workspaceName: string): string =>
			...
			permanentWorktreeSession: t("workspaceTree.status.permanentWorktreeSession"),
			workspaceActionsAria: (workspaceName: string): string =>
	...
	workspaceTreeOrderRef.current = {
		...workspaceTreeOrder,
		expandedSectionKeys: expandedSectionKeysRef.current,
	...
	function setCanonicalWorkspaceTreeOrder(nextOrder: WorkspaceTreeOrderPreferences): void {
		...
		expandedSectionKeysRef.current = nextOrder.expandedSectionKeys;
		expandedWorkspaceIdsRef.current = nextOrder.expandedWorkspaceIds;
		setExpandedSectionKeys(nextOrder.expandedSectionKeys);
	...
	function enqueueWorkspaceTreeOrderSave(nextOrder: WorkspaceTreeOrderPreferences): void {
		...
		const payload = {
			...
			sessionIdsByWorkspace: Object.fromEntries(
				Object.entries(nextOrder.sessionIdsByWorkspace).map(([workspaceId, sessionIds]): [string, string[]] => [
					workspaceId,
					[...sessionIds],
				]),
			),
			...
			sectionOrder: [...nextOrder.sectionOrder],
				...
				try {
					...
					if (isMountedRef.current && revision === orderSaveRevisionRef.current) {
						setCanonicalWorkspaceTreeOrder(
							reconcileWorkspaceTreeOrder(storedOrder, workspacesRef.current, sessionsRef.current),
						);
					}
	...
	function persistExpandedTreeState(
		nextExpandedSectionKeys: WorkspaceTreeSectionKey[],
		...
		const nextOrder: WorkspaceTreeOrderPreferences = {
			...workspaceTreeOrderRef.current,
			expandedSectionKeys: nextExpandedSectionKeys,
			expandedWorkspaceIds: nextExpandedWorkspaceIds,
		};
		workspaceTreeOrderRef.current = nextOrder;
		expandedSectionKeysRef.current = nextExpandedSectionKeys;
		expandedWorkspaceIdsRef.current = nextExpandedWorkspaceIds;
		setExpandedSectionKeys(nextExpandedSectionKeys);
	...
	function persistExpandedWorkspaceIds(nextExpandedWorkspaceIds: string[]): void {
		persistExpandedTreeState(expandedSectionKeysRef.current, nextExpandedWorkspaceIds);
	}

	function persistExpandedSectionKeys(nextExpandedSectionKeys: WorkspaceTreeSectionKey[]): void {
		persistExpandedTreeState(nextExpandedSectionKeys, expandedWorkspaceIdsRef.current);
	}

	function ensureSectionOpen(sectionKey: WorkspaceTreeSectionKey): void {
		const currentKeys: WorkspaceTreeSectionKey[] = expandedSectionKeysRef.current;
		if (currentKeys.includes(sectionKey)) {
			return;
		}
		persistExpandedSectionKeys([...currentKeys, sectionKey]);
	}
			...
			setSessions((currentSessions: SessionMetadata[]): SessionMetadata[] =>
				currentSessions.map(
					(currentSession: SessionMetadata): SessionMetadata =>
						currentSession.id === metadata.id ? metadata : currentSession,
				),
			);
					...
					: getSessionProjectWorkspaceId(metadata) !== undefined &&
						  workspaces.some(
								(workspace: WorkspaceConfig): boolean =>
									workspace.id === getSessionProjectWorkspaceId(metadata),
						  )
						? "projects"
						: "recent";
	...
	async function handleConfirmDeleteWorktree(): Promise<void> {
		...
		try {
			...
			const metadata: SessionMetadata = await onSessionWorktreeDelete(deleteWorktreeTarget);
			setSessions((currentSessions): SessionMetadata[] =>
				currentSessions.map((session): SessionMetadata => (session.id === metadata.id ? metadata : session)),
			);
		...
		try {
			...
			const nextSessions: SessionMetadata[] = sessionsRef.current.map(
				(session: SessionMetadata): SessionMetadata =>
					session.id === result.metadata.id ? result.metadata : session,
			);
	...
	async function handleOpenSessionWorkspaceInExplorer(session: SessionMetadata): Promise<void> {
		...
		try {
			setWorkspaceError(null);
			await window.electronAPI.workspaceFs.openWorkspaceDirectory(rootPath);
		} catch (error: unknown) {
	...
	useEffect((): (() => void) => {
		...
		return (): void => {
			const pendingOrder: WorkspaceTreeOrderPreferences | null =
				expansionSaveTimerRef.current === null ? null : workspaceTreeOrderRef.current;
			clearScheduledExpansionSave();
	...
	useEffect((): (() => void) => {
		...
		async function loadWorkspaceTree(): Promise<void> {
			try {
				...
				expandedSectionKeysRef.current = reconciledOrder.expandedSectionKeys;
				expandedWorkspaceIdsRef.current = reconciledOrder.expandedWorkspaceIds;
				setWorkspaces(workspaceList.workspaces);
				setSessions(visibleSessions);
				setExpandedSectionKeys(reconciledOrder.expandedSectionKeys);
	...
	const orderedPinnedSessions: SessionMetadata[] = useMemo((): SessionMetadata[] => {
		return sortSessionsByTreeOrder(sessionGroups.pinnedSessions, effectiveWorkspaceTreeOrder.pinnedSessionIds);
	...
	const orderedRecentSessions: SessionMetadata[] = useMemo((): SessionMetadata[] => {
		return sortSessionsByTreeOrder(sessionGroups.recentSessions, effectiveWorkspaceTreeOrder.recentSessionIds);
	...
	const projectTreeData: ProjectTreeNode[] = useMemo((): ProjectTreeNode[] => {
		return createProjectTreeData(orderedWorkspaces, orderedProjectSessions, {
			...
			onWorkspaceDragLeave: (workspaceId: string): void => {
				setDropTargetWorkspaceId((currentId: string | null): string | null =>
	...
	const expandedTreeKeys: string[] = [
		...expandedSectionKeys.map((sectionKey: WorkspaceTreeSectionKey): string => `section:${sectionKey}`),
		...expandedWorkspaceIds.map((workspaceId: string): string => `workspace:${workspaceId}`),
	];
	const handleProjectTreeExpand: NonNullable<TreeProps<ProjectTreeNode>["onExpand"]> = (expandedKeys): void => {
		const normalizedKeys: string[] = expandedKeys.map(String);
		...
		const expandedWorkspaces: string[] = normalizedKeys.flatMap((key: string): string[] => {
			return key.startsWith("workspace:") ? [key.slice("workspace:".length)] : [];
		});
		persistExpandedTreeState(expandedSections, expandedWorkspaces);
	};
	const handleProjectTreeSelect: NonNullable<TreeProps<ProjectTreeNode>["onSelect"]> = (
		_selectedKeys,
		info,
	): void => {
		const node: ProjectTreeNode = info.node;
		const selectedKey: string = String(node.key);
		if (node.kind === "empty" || node.kind === "section") {
			return;
		}
	...
	const canDropTreeNode = (dragNode: ProjectTreeNode, dropNode: ProjectTreeNode, dropToGap: boolean): boolean =>
		canDropWorkspaceTreeNode(dragNode, dropNode, dropToGap, effectiveWorkspaceTreeOrder);
	...
	const isTreeNodeDraggable = (node: TreeDataNode): boolean => {
		...
		if (treeNode.kind === "section") {
			return effectiveWorkspaceTreeOrder.sectionOrder.length > 1;
		}
	...
	const sectionTreeData: ProjectTreeNode[] = useMemo((): ProjectTreeNode[] => {
		...
		function createSection(
			...
			return {
				...
				className: styles.sectionTreeNode,
				kind: "section",
				sectionKey,
				selectable: false,
				children,
		...
		const sections: Record<WorkspaceTreeSectionKey, ProjectTreeNode> = {
			pinned: createSection("pinned", labels.pinned, pinnedTreeData),
			projects: createSection(
				"projects",
				labels.projects,
				projectTreeData,
				createSectionAction(labels.newProject, (): void => setIsCreateProjectOpen(true), true),
			),
			recent: createSection(
				"recent",
				labels.recent,
				recentTreeData,
				createSectionAction(labels.newSession, (): void => {
					ensureSectionOpen("recent");
					onNewSession?.();
				}),
			),
		};
		return effectiveWorkspaceTreeOrder.sectionOrder.map(
			(sectionKey: WorkspaceTreeSectionKey): ProjectTreeNode => sections[sectionKey],
		);
	}, [
		effectiveWorkspaceTreeOrder.sectionOrder,
		labels.newProject,
		labels.newSession,
		labels.pinned,
		labels.projects,
	...
	useEffect((): void => {
		...
		setSessions((currentSessions: SessionMetadata[]): SessionMetadata[] => {
			...
			const existingIndex: number = currentSessions.findIndex(
				(session: SessionMetadata): boolean => session.id === sessionUpdate.id,
			);
			if (existingIndex < 0) {
	...
	return (
		<div className={styles.workspaceTreeRegion} data-studio-workspace-tree="true">
					...
					setSessions((currentSessions): SessionMetadata[] =>
						currentSessions.map(
							(currentSession): SessionMetadata =>
								getSessionProjectWorkspaceId(currentSession) === updatedWorkspace.id
									? {
											...currentSession,
						...
						currentWorkspaces.some(
							...
							? currentWorkspaces.map(
									(workspace: WorkspaceConfig): WorkspaceConfig =>
										workspace.id === createdWorkspace.id ? createdWorkspace : workspace,
								)
			...
			<Modal
				...
				onCancel={(): void => {
					if (deletingWorktreeSessionId === null) setDeleteWorktreeTarget(null);
				}}
			>
				<Typography.Paragraph>{labels.deleteWorktreeBody}</Typography.Paragraph>
			</Modal>
