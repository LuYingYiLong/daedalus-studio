...
function getStepTitle(
	...
	return (
		step.title.trim() ||
		step.text?.trim() ||
		t("workflowTodo.stepFallback", { index: index + 1 })
	);
}
...
function getFinishedStepCount(steps: WorkflowTodoStep[]): number {
	return steps.filter((step: WorkflowTodoStep): boolean =>
		isFinishedStep(step.status),
	).length;
}

export function getWorkflowTodoProgress(steps: WorkflowTodoStep[]): {
	finished: number;
	percent: number;
} {
...
export function WorkflowTodoStepList({
	steps,
}: WorkflowTodoStepListProps): React.JSX.Element {
	const { t } = useTranslation();
	return (
		<div className={styles.detailSteps}>
			{steps.map(
				(step: WorkflowTodoStep, index: number): React.ReactNode => {
					const title: string = getStepTitle(step, index, t);
					const description: string | undefined =
						step.text !== undefined && step.text !== title
							? step.text
							: undefined;
					return (
						<div key={step.id} className={styles.detailStep}>
							<Icon
								name={getWorkflowTodoIconName(step.status)}
...
function FloatingWorkflowTodoPanel({
	snapshot,
	fileChangeSummary,
	onDismiss,
}: FloatingWorkflowTodoPanelProps): React.JSX.Element | null {
	const { t } = useTranslation();
	...
	const { finished: finishedStepCount, percent } =
		getWorkflowTodoProgress(steps);
	...
	return (
		<div
			...
			aria-label={t("workflowTodo.aria.progress")}
		>
			<Popover
				trigger="hover"
				placement="top"
			...
			>
				...
				>
					<Progress
						...
						strokeWidth={10}
					/>
					<Typography.Text className={styles.phaseText}>
			...
			<span
				...
				aria-label={t("workflowTodo.aria.fileChanges")}
			>
