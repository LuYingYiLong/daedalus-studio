...
function getSourceFolderLabel(sourceFolder: WorkspaceSourceFolder): string {
	const normalizedPath: string = sourceFolder.path
		.replaceAll("\\", "/")
		.replace(/\/$/u, "");
...
function SkillsSettingsPage(): React.JSX.Element | null {
	...
	const [viewSelection, setViewSelection] =
		useState<SkillViewSelection>("all");
	...
	const [npxCandidates, setNpxCandidates] = useState<
		NpxSkillCandidate[] | null
	>(null);
	const [npxSelectedPaths, setNpxSelectedPaths] = useState<string[]>([]);
	const [npxTargetSource, setNpxTargetSource] =
		useState<SkillInstallSource>("personal");
	...
	const [npxImportSummary, setNpxImportSummary] =
		useState<NpxImportSummary | null>(null);
	const [skillEditor, setSkillEditor] = useState<SkillEditorState | null>(
		null,
	);
	const [isSkillEditorLoading, setIsSkillEditorLoading] =
		useState<boolean>(false);
	const [isSkillEditorSaving, setIsSkillEditorSaving] =
		useState<boolean>(false);
	const [skillEditorError, setSkillEditorError] = useState<string | null>(
		null,
	);
	const addItems: MenuProps["items"] = useMemo(
		(): MenuProps["items"] => [
			{
				key: "npx",
	...
	const installScopeOptions: Array<{
		value: SkillInstallSource;
		label: string;
	}> = useMemo(
		(): Array<{ value: SkillInstallSource; label: string }> => [
	...
	const workspaceOptions: Array<{ value: string; label: string }> =
		useMemo((): Array<{ value: string; label: string }> => {
	...
	const viewSelectionOptions = useMemo(
		(): SelectProps["options"] => [
	...
	const sourceFolderOptions: Array<{ value: string; label: string }> =
		useMemo((): Array<{ value: string; label: string }> => {
			return (selectedWorkspace?.sourceFolders ?? []).map(
				(
	...
	function targetForSkill(skill: SkillSummary): SkillTarget {
		...
		return {
			...(skill.workspaceId === undefined
				? selectedCatalogTarget()
				: { workspaceId: skill.workspaceId }),
			...(skill.sourceFolderId === undefined
				? {}
	...
	function sourceFolderLabel(
		...
		const sourceWorkspace: WorkspaceConfig | null =
		...
		const sourceFolder: WorkspaceSourceFolder | undefined =
			sourceWorkspace?.sourceFolders.find(
				(candidate: WorkspaceSourceFolder): boolean =>
					candidate.id === sourceFolderId,
			);
		return sourceFolder === undefined
			? sourceFolderId
			: getSourceFolderLabel(sourceFolder);
	}
	...
	const loadSkillsForView = useCallback(
		async (
			selection: SkillViewSelection,
			availableWorkspaces: WorkspaceConfig[],
		): Promise<SkillSummary[]> => {
			if (selection === "personal") {
				return (await fetchSkills()).skills.filter(
			...
			if (selection === "all") {
				const results: SkillListResult[] = await Promise.all([
					...
					...availableWorkspaces.map(
						(
							...
							fetchSkills({ workspaceId: workspace.id }),
					),
				]);
				const skillsByIdentity: Map<string, SkillSummary> = new Map();
				for (const skill of results[0]!.skills.filter(
					(candidate: SkillSummary): boolean =>
						candidate.source === "personal",
				)) {
					skillsByIdentity.set(
						`${skill.workspaceId ?? "global"}\u0000${skill.ref}`,
						skill,
					);
				}
				for (const result of results.slice(1)) {
					for (const skill of result.skills.filter(
						...
						skillsByIdentity.set(
							`${skill.workspaceId ?? "unknown"}\u0000${skill.ref}`,
							skill,
						);
					}
				}
			...
			return (await fetchSkills({ workspaceId })).skills.filter(
				(skill: SkillSummary): boolean => skill.source === "project",
			);
		},
		[],
	);
				...
				if (!cancelled) {
					setErrorMessage(
						error instanceof Error
							? error.message
							: t("settings.skills.errors.loadWorkspaces"),
					);
				}
			} finally {
				if (!cancelled) {
	...
	useEffect((): (() => void) => {
		...
		async function loadSkills(): Promise<void> {
			try {
				...
				const nextSkills: SkillSummary[] = await loadSkillsForView(
					viewSelection,
					workspaces,
				);
				if (!cancelled) {
	...
	const customSkills: SkillSummary[] = useMemo((): SkillSummary[] => {
		return skills.filter(
			(skill: SkillSummary): boolean => skill.source !== "builtin",
		);
	}, [skills]);
	...
	const importableNpxCandidates: NpxSkillCandidate[] =
		useMemo((): NpxSkillCandidate[] => {
			return (npxCandidates ?? []).filter(
				(candidate: NpxSkillCandidate): boolean => {
					return !skills.some((skill: SkillSummary): boolean => {
						return (
							skill.source === npxTargetSource &&
							skill.slug === candidate.slug &&
							(npxTargetSource !== "project" ||
								skill.sourceFolderId === selectedSourceFolderId)
						);
					});
				},
			);
		}, [npxCandidates, npxTargetSource, selectedSourceFolderId, skills]);
	const selectedNpxCandidates: NpxSkillCandidate[] =
		useMemo((): NpxSkillCandidate[] => {
			const selectedPaths = new Set<string>(npxSelectedPaths);
			return importableNpxCandidates.filter(
				(candidate: NpxSkillCandidate): boolean =>
	...
	async function openInstallDialog(kind: SkillInstallKind): Promise<void> {
		try {
			...
			setErrorMessage(
				error instanceof Error
					? error.message
					: t("settings.skills.errors.selectSource"),
			);
		}
	}
	...
	async function openNpxImportDialog(): Promise<void> {
		...
		try {
			const candidates: NpxSkillCandidate[] =
				await window.electronAPI.skillCli.listGlobalCodexSkills();
			setNpxCandidates(candidates);
			setNpxSelectedPaths(
				candidates
					.filter((candidate: NpxSkillCandidate): boolean => {
						return !skills.some(
							(skill: SkillSummary): boolean =>
								skill.source === "personal" &&
								skill.slug === candidate.slug,
					...
					.map(
						(candidate: NpxSkillCandidate): string =>
							candidate.path,
					),
			);
		} catch (error: unknown) {
			setNpxImportError(
				error instanceof Error
					? error.message
					: t("settings.skills.errors.npxDiscover"),
			);
		} finally {
	...
	async function handleConfirmNpxImport(): Promise<void> {
		...
		const summary: NpxImportSummary = {
			installed: 0,
			skipped:
				(npxCandidates?.length ?? 0) - importableNpxCandidates.length,
			failed: [],
		};
		try {
				...
				try {
					...
					summary.failed.push({
						...
						message:
							error instanceof Error
								? error.message
								: t("settings.skills.errors.install"),
					});
				}
			}
	...
	async function handleSetEnabled(
		...
		try {
			...
			setErrorMessage(
				error instanceof Error
					? error.message
					: t("settings.skills.errors.update"),
			);
		} finally {
			setBusyRef(null);
		}
	...
	async function openSkillEditor(skill: SkillSummary): Promise<void> {
		...
		try {
			const result: { ref: string; content: string } =
				await fetchSkillContent(skill.ref, targetForSkill(skill));
			setSkillEditor(
				(current: SkillEditorState | null): SkillEditorState | null => {
					return current?.skill.ref === result.ref
			...
			setSkillEditorError(
				error instanceof Error
					? error.message
					: t("settings.skills.errors.loadContent"),
			);
		} finally {
	...
	async function saveSkillEditor(): Promise<void> {
		...
		try {
			await updateSkillContent(
				skillEditor.skill.ref,
				skillEditor.content,
				targetForSkill(skillEditor.skill),
			);
			await refreshVisibleSkills();
			setSkillEditor(null);
		} catch (error: unknown) {
			setSkillEditorError(
				error instanceof Error
					? error.message
					: t("settings.skills.errors.updateContent"),
			);
		} finally {
		...
		modal.confirm({
			...
			content: t(
				skill.source === "project"
					...
					: "settings.skills.confirm.delete.description",
				{ name: skill.name },
			),
			...
			async onOk(): Promise<void> {
				try {
					...
					setErrorMessage(
						error instanceof Error
							? error.message
							: t("settings.skills.errors.delete"),
					);
				} finally {
					setBusyRef(null);
				}
	...
	return (
		<section className={styles.page}>
				...
				<Flex gap="small" className={styles.toolbar}>
					...
					<Select
						...
						onChange={(value: string): void =>
							handleViewSelectionChange(value)
						}
					/>
					<Dropdown
						menu={{
					...
					<Empty
						description={
							customSkills.length === 0
								...
								: t("settings.skills.empty.noMatches")
						}
					/>
				) : (
					filteredSkills.map(
						(skill: SkillSummary): React.JSX.Element => {
							const isBusy: boolean =
								busyRef === getSkillIdentity(skill);
							return (
								<div
									key={skill.ref}
										...
										>
											...
											<Tooltip
												title={
													skill.enabled
														? t(
																"settings.common.disable",
															)
														: t(
																"settings.common.enable",
															)
												}
											>
												<Switch
			...
			<Modal
				...
				title={
					skillEditor === null
						? t("settings.skills.editor.title")
						: t("settings.skills.editor.titleWithName", {
				...
				okButtonProps={{
					disabled:
						isSkillEditorLoading ||
						skillEditor?.content.trim().length === 0,
				}}
				cancelButtonProps={{ disabled: isSkillEditorSaving }}
				destroyOnHidden={true}
				onOk={(): void => {
					void saveSkillEditor();
				}}
				onCancel={(): void => {
				...
				<div className={styles.skillEditor}>
					...
					<Spin spinning={isSkillEditorLoading}>
						<Input.TextArea
							...
							spellCheck={false}
							disabled={
								isSkillEditorLoading || isSkillEditorSaving
							}
							onChange={(
								...
								setSkillEditor(
									(
										current: SkillEditorState | null,
									): SkillEditorState | null =>
										current === null
											? null
											: { ...current, content },
								);
					...
					<Typography.Text type="secondary">
						{t("settings.skills.editor.description")}
					</Typography.Text>
				</div>
			</Modal>
			...
			<Modal
				title={
					pendingInstall === null
						? t("settings.skills.install.title")
						: t(
								pendingInstall.kind === "zip"
									? "settings.skills.install.fromZip"
									: "settings.skills.install.fromFolder",
							)
				}
			...
			>
				{pendingInstall !== null ? (
					<div className={styles.installForm}>
						<Typography.Text className={styles.skillSummary}>
							{pendingInstall.path}
						</Typography.Text>
						<Select
						...
						{pendingInstall.source === "project" ? (
							<>
								<Select
									...
									placeholder={t(
										"settings.skills.target.workspacePlaceholder",
									)}
									onChange={(value: string): void =>
										handleWorkspaceTargetChange(value)
									}
								/>
								<Select
									...
									placeholder={t(
										"settings.skills.target.sourceFolderPlaceholder",
									)}
								...
								<Typography.Text type="secondary">
									{t(
										"settings.skills.target.projectPathHint",
									)}
								</Typography.Text>
							</>
						) : null}
			...
			<Modal
				...
				okButtonProps={{
					disabled:
						selectedNpxCandidates.length === 0 ||
						isNpxLoading ||
						npxImportError !== null ||
						(npxTargetSource === "project" &&
							selectedProjectTarget() === null),
			...
			>
				<div className={styles.npxImportForm}>
					<Typography.Text type="secondary">
						{t("settings.skills.npx.description")}
					</Typography.Text>
					...
					{npxImportSummary !== null ? (
						<Alert
							type={
								npxImportSummary.failed.length === 0
									? "success"
									: "warning"
							}
							showIcon={true}
					...
					{npxCandidates !== null && npxCandidates.length === 0 ? (
						<Empty
							...
							description={t("settings.skills.npx.empty")}
						/>
					) : null}
					{npxCandidates !== null && npxCandidates.length > 0 ? (
						<>
							...
							<Checkbox
								checked={
									importableNpxCandidates.length > 0 &&
									selectedNpxCandidates.length ===
										importableNpxCandidates.length
								}
								indeterminate={
									selectedNpxCandidates.length > 0 &&
									selectedNpxCandidates.length <
								...
								onChange={(event): void =>
									toggleAllNpxCandidates(event.target.checked)
								}
							>
							...
							<div className={styles.npxSkillList}>
								{npxCandidates.map(
									(
										candidate: NpxSkillCandidate,
									): React.JSX.Element => {
										const alreadyInstalled: boolean =
											!importableNpxCandidates.some(
												(
													item: NpxSkillCandidate,
												): boolean =>
													item.path ===
													candidate.path,
											);
										return (
											<div
												key={candidate.path}
												className={styles.npxSkillItem}
											>
												<Checkbox
													checked={
														npxSelectedPaths.includes(
															candidate.path,
														) && !alreadyInstalled
													}
													disabled={
												...
												{alreadyInstalled ? (
													<Tag>
														{t(
															"settings.skills.npx.alreadyInstalled",
														)}
													</Tag>
												) : null}
											</div>
