...
import { useComputerDeveloperMode, useComputerState } from "@/features/computer-observation/useComputerState";
import styles from "./ComputerObservationSettingsPage.module.css";

export default function ComputerObservationSettingsPage({ isActive }: { isActive: boolean }): React.JSX.Element {
	...
	return (
		<section className={styles.page} data-testid="computer-observation-settings">
					...
					{api && (
						<>
							...
							{(error || state?.error) && (
								<Alert
									type="warning"
									title={error ? t("computer.saveFailed") : t("computer.resourcesMissing")}
								/>
							)}
						</>
					)}
