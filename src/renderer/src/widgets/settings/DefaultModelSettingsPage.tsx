...
function isConfiguredModelRef(
	...
	ref: ProviderTaskModelRef,
): boolean {
	return getConfiguredProviders(selection).some(
		(provider: ProviderModelSelectionProvider): boolean => {
			return (
...
function createModelSelectOptions(
	selection: ProviderModelSelection | null,
	filterModel?: (model: ProviderModelInfo) => boolean,
): SelectProps["options"] {
	return getConfiguredProviders(selection)
...
function DefaultModelSettingsPage({
	onSelectionChange,
}: DefaultModelSettingsPageProps): React.JSX.Element | null {
				...
				if (!cancelled) {
					setErrorMessage(
						error instanceof Error
							? error.message
							: t("settings.defaultModel.errors.load"),
					);
				}
			} finally {
				if (!cancelled) {
	...
	async function handleRoutingChange(
		...
		const nextModelRef: ProviderTaskModelRef | null =
			value === undefined ? null : decodeModelRef(value);
		...
		const configuredProviders: ProviderModelSelectionProvider[] =
			getConfiguredProviders(selection);
		const persistenceProvider: ProviderModelSelectionProvider | undefined =
			(nextModelRef === null
				? undefined
				: configuredProviders.find(
						(provider: ProviderModelSelectionProvider): boolean =>
							provider.provider === nextModelRef.provider,
					)) ??
			configuredProviders.find(
				(provider: ProviderModelSelectionProvider): boolean =>
		...
		const persistenceModel: string | undefined =
			nextModelRef?.model ??
		...
		try {
			...
			const nextSelection: ProviderModelSelection =
				await saveProviderConfig(saveParams);
			...
			setErrorMessage(
				error instanceof Error
					? error.message
					: t("settings.defaultModel.errors.save"),
			);
		} finally {
			setSavingKey(null);
		}
		...
		return (
			<section className={styles.page}>
				<Alert
					...
					description={
						errorMessage ??
						t("settings.defaultModel.errors.noSelection")
					}
				/>
			</section>
		);
	...
	const hasConfiguredProviders: boolean =
		getConfiguredProviders(selection).length > 0;
	...
	return (
		<section className={styles.page}>
			<div className={styles.cards}>
				...
				{ROUTING_OPTIONS.map(
					(option: RoutingOption): React.JSX.Element => {
						const routedModel: ProviderTaskModelRef | null =
							selection.modelRouting[option.key];
						const value: string | undefined =
							routedModel === null ||
							!isConfiguredModelRef(selection, routedModel)
								? undefined
							...
							<article key={option.key} className={styles.card}>
								...
								<div className={styles.controlRow}>
									<Select
										...
										allowClear={{
											clearIcon: <Icon name="clear" />,
										}}
										placeholder={
											!hasConfiguredProviders
												? t(
														"settings.defaultModel.configureProvider",
													)
												: option.placeholderKey ===
													  undefined
													? t(
															"settings.defaultModel.selectModel",
														)
