...
function cacheDirectoryEntries(
	cache: Map<string, FileTreeNode[]>,
	key: string,
...
function getSourceFolderLabel(folder: WorkspaceSourceFolder): string {
	return (
		folder.path.replaceAll("\\", "/").split("/").filter(Boolean).at(-1) ??
		folder.path
	);
}
		...
		icon: (
			<Icon
				name={
					entry.kind === "folder"
						? "folder"
						: getFileIconName(entry.relativePath)
				}
			/>
		),
	};
...
function replaceTreeChildren(
	...
	const nextNodes: FileTreeNode[] = nodes.map(
		(node: FileTreeNode): FileTreeNode => {
			if (node.key === key) {
				changed = true;
				return { ...node, children };
			}
			if (node.children === undefined || !key.startsWith(`${node.key}/`))
				return node;
			const nextChildren: FileTreeNode[] = replaceTreeChildren(
				node.children,
				key,
				children,
			);
...
function normalizeEditorSplitSize(value: number): number {
	...
	return Math.min(
		FILE_PANEL_MAX_EDITOR_SPLIT,
		Math.max(FILE_PANEL_MIN_EDITOR_SPLIT, value),
	);
}

function getEditorSplitSizeFromPixels(sizes: number[]): number | null {
	const totalSize: number = sizes.reduce(
		(total: number, size: number): number => total + size,
		0,
	);
...
export function FilePanel({
	...
	const directoryRequestsRef = useRef<Map<string, Promise<FileTreeNode[]>>>(
		new Map(),
	);
	const layoutRef = useRef<FilePanelLayoutPreferences>(layout);
	const onLayoutChangeRef =
		useRef<FilePanelProps["onLayoutChange"]>(onLayoutChange);
	const expandedPathsPersistTimerRef = useRef<number | null>(null);
	const pendingExpandedPathsRef = useRef<Record<string, string[]> | null>(
		null,
	);
	const [visualEditorSplitSize, setVisualEditorSplitSize] = useState<number>(
		() => normalizeEditorSplitSize(layout.splitSize),
	);
	...
	const selectedSourceFolder: WorkspaceSourceFolder | null =
		useMemo((): WorkspaceSourceFolder | null => {
			if (sourceFolders.length === 0) return null;
			return (
				sourceFolders.find(
					(folder: WorkspaceSourceFolder): boolean =>
						folder.id === layout.selectedSourceFolderId,
				) ??
				sourceFolders.find(
					(folder: WorkspaceSourceFolder): boolean =>
						folder.id === workspace?.primarySourceFolderId,
				) ??
				sourceFolders[0] ??
				null
			);
		}, [
			layout.selectedSourceFolderId,
			sourceFolders,
			workspace?.primarySourceFolderId,
		]);
	const activeTab: FileTabPreferences | null =
		layout.tabs.find(
			(tab: FileTabPreferences): boolean =>
				tab.key === layout.activeTabKey,
		) ??
		layout.tabs[0] ??
		null;
	const activeBuffer: FileBuffer | null =
		activeTab === null ? null : (buffers[activeTab.key] ?? null);
	const isDirty: boolean = activeBuffer?.isDirty === true;
	const editorSplitSize: number = visualEditorSplitSize;

	const patchLayout = useCallback(
		(patch: Partial<FilePanelLayoutPreferences>): void => {
			const currentLayout: FilePanelLayoutPreferences = layoutRef.current;
			const nextLayout: FilePanelLayoutPreferences = {
				...currentLayout,
				...patch,
				splitSize: normalizeEditorSplitSize(
					patch.splitSize ?? currentLayout.splitSize,
				),
			};
	...
	const handleSplitterResize = useCallback((sizes: number[]): void => {
		const nextSplitSize: number | null =
			getEditorSplitSizeFromPixels(sizes);
		if (nextSplitSize !== null) setVisualEditorSplitSize(nextSplitSize);
	}, []);

	const handleSplitterResizeEnd = useCallback(
		(sizes: number[]): void => {
			const nextSplitSize: number | null =
				getEditorSplitSizeFromPixels(sizes);
	...
	const flushExpandedPaths = useCallback((): void => {
		...
		const pendingExpandedPaths: Record<string, string[]> | null =
			pendingExpandedPathsRef.current;
		pendingExpandedPathsRef.current = null;
		if (pendingExpandedPaths !== null)
			patchLayout({ expandedPathsBySourceFolder: pendingExpandedPaths });
	}, [patchLayout]);

	const scheduleExpandedPathsPersist = useCallback(
		(sourceFolderId: string, keys: React.Key[]): void => {
			const currentExpandedPaths: Record<string, string[]> =
				pendingExpandedPathsRef.current ??
			...
			if (expandedPathsPersistTimerRef.current !== null)
				window.clearTimeout(expandedPathsPersistTimerRef.current);
			expandedPathsPersistTimerRef.current = window.setTimeout(
				flushExpandedPaths,
				EXPANDED_PATHS_PERSIST_DELAY_MS,
			);
		},
		[flushExpandedPaths],
	);
	...
	const loadBuffer = useCallback(
		async (
			...
			const sourceFolder: WorkspaceSourceFolder | undefined =
				sourceFolders.find(
					(folder: WorkspaceSourceFolder): boolean =>
						folder.id === tab.sourceFolderId,
				);
			if (sourceFolder === undefined) return;
			const runtimeKey: string = getBufferKey(sessionId, panelKey, tab);
			const existing: FileBuffer | undefined =
				RUNTIME_BUFFERS.get(runtimeKey);
			...
			setBuffers((current) => ({
				...current,
				[tab.key]: {
					...(current[tab.key] ?? createEmptyBuffer()),
					loading: true,
					error: null,
				},
			}));
			try {
				const result =
					await window.electronAPI.workspaceFs.readTextFile({
						workspaceRoot: sourceFolder.path,
						filePath: tab.relativePath,
					});
				const media = result.readable
					? null
					: await window.electronAPI.workspaceFs.createMediaUrl({
							workspaceRoot: sourceFolder.path,
							filePath: tab.relativePath,
						});
				...
				if (
					!layoutRef.current.tabs.some(
						(candidate: FileTabPreferences): boolean =>
	...
	const handleEditorContentChange = useCallback(
		(tab: FileTabPreferences, nextContent: string): void => {
			setBuffers((current) => {
				const currentBuffer: FileBuffer | undefined = current[tab.key];
				if (
					currentBuffer === undefined ||
					currentBuffer.content === nextContent
				)
					return current;
				const savedContent: string = currentBuffer.isDirty
					? (currentBuffer.savedContent ?? currentBuffer.content)
				...
				RUNTIME_BUFFERS.set(
					getBufferKey(sessionId, panelKey, tab),
					next,
				);
	...
	useEffect((): void => {
		if (
			...
			layout.selectedSourceFolderId === selectedSourceFolder.id
		)
			return;
	...
	const loadDirectory = useCallback(
		async (
			...
			const cacheKey: string = getDirectoryCacheKey(
				sourceFolder,
				relativePath,
			);
			const cachedEntries: FileTreeNode[] | undefined =
				directoryCacheRef.current.get(cacheKey);
			...
			const pendingRequest: Promise<FileTreeNode[]> | undefined =
				directoryRequestsRef.current.get(cacheKey);
			if (pendingRequest !== undefined) return pendingRequest;
			const request: Promise<FileTreeNode[]> =
			...
			void window.electronAPI.workspaceFs
				...
				.then((result): void => {
					if (searchRequestRef.current === requestId)
						setSearchResults(result.entries.map(createTreeNode));
				})
	...
	const pinTab = useCallback(
		(key: string): void => {
			const targetTab: FileTabPreferences | undefined = layout.tabs.find(
				(tab: FileTabPreferences): boolean => tab.key === key,
			);
			if (targetTab === undefined || targetTab.pinned) return;
			patchLayout({
				previewTabKey:
					layout.previewTabKey === key ? null : layout.previewTabKey,
				tabs: layout.tabs.map(
					(tab: FileTabPreferences): FileTabPreferences =>
	...
	const openEntry = useCallback(
		(entry: WorkspaceFsEntry, pinned: boolean): void => {
			...
			const existing: FileTabPreferences | undefined = layout.tabs.find(
				(tab: FileTabPreferences): boolean => tab.key === key,
			);
			if (existing !== undefined) {
				patchLayout({
					...
					...(pinned
						? {
								previewTabKey:
									layout.previewTabKey === key
										? null
										: layout.previewTabKey,
								tabs: layout.tabs.map(
									(
										tab: FileTabPreferences,
									): FileTabPreferences =>
			...
			const nextTab: FileTabPreferences = {
				key,
				sourceFolderId: selectedSourceFolder.id,
				relativePath: normalizeRelativePath(entry.relativePath),
				pinned,
			};
			const previewKey: string | null = pinned
				? layout.previewTabKey
				: key;
			const replaceIndex: number =
				pinned || layout.previewTabKey === null
					? -1
					: layout.tabs.findIndex(
							(tab: FileTabPreferences): boolean =>
								tab.key === layout.previewTabKey && !tab.pinned,
						);
			const nextTabs: FileTabPreferences[] = [...layout.tabs];
			if (replaceIndex >= 0) {
				const replacedTab: FileTabPreferences | undefined =
					nextTabs[replaceIndex];
				if (replacedTab !== undefined) {
					RUNTIME_BUFFERS.delete(
						getBufferKey(sessionId, panelKey, replacedTab),
					);
					setBuffers((current) => {
						const next = { ...current };
	...
	const removeTab = useCallback(
		(tab: FileTabPreferences): void => {
			...
			const index: number = layout.tabs.findIndex(
				(candidate: FileTabPreferences): boolean =>
					candidate.key === tab.key,
			);
			const nextTabs: FileTabPreferences[] = layout.tabs.filter(
				(candidate: FileTabPreferences): boolean =>
					candidate.key !== tab.key,
			);
			patchLayout({
				tabs: nextTabs,
				activeTabKey:
					layout.activeTabKey === tab.key
						? (nextTabs[Math.max(0, index - 1)]?.key ??
							nextTabs[0]?.key ??
							null)
						: layout.activeTabKey,
				previewTabKey:
					layout.previewTabKey === tab.key
						? null
						: layout.previewTabKey,
			});
		},
	...
	const saveTab = useCallback(
		async (tab: FileTabPreferences): Promise<boolean> => {
			const sourceFolder: WorkspaceSourceFolder | undefined =
				sourceFolders.find(
					(folder: WorkspaceSourceFolder): boolean =>
						folder.id === tab.sourceFolderId,
			...
			setBuffers((current) => ({
				...current,
				[tab.key]: { ...buffer, saving: true, error: null },
			}));
			try {
				const result =
					await window.electronAPI.workspaceFs.writeTextFile({
						workspaceRoot: sourceFolder.path,
						filePath: tab.relativePath,
				...
				const conflict: boolean = String(error).includes(
				...
				void messageApi.error(
					conflict ? t("files.conflict") : String(error),
				);
				return false;
			}
		},
		...
		async (
			...
			if (content === undefined && entry !== undefined) {
				...
				if (content === undefined) {
					const copied =
						await window.electronAPI.workspaceFs.saveFileAs({
							workspaceRoot: sourceFolder.path,
							filePath: entry.relativePath,
						});
					if (copied.saved)
						void messageApi.success(t("files.savedAs"));
					return;
				}
			}
			if (content === undefined) return;
			const result = await window.electronAPI.workspaceFs.saveTextFileAs({
				workspaceRoot: sourceFolder.path,
				filePath: tab.relativePath,
				content,
			});
	...
	useEffect((): void => {
		const dirtyUnpinnedTab: FileTabPreferences | undefined =
			layout.tabs.find((tab: FileTabPreferences): boolean => {
				const buffer: FileBuffer | undefined = buffers[tab.key];
		...
		patchLayout({
			previewTabKey:
				layout.previewTabKey === dirtyUnpinnedTab.key
					? null
					: layout.previewTabKey,
			tabs: layout.tabs.map(
				(tab: FileTabPreferences): FileTabPreferences =>
					tab.key === dirtyUnpinnedTab.key
						? { ...tab, pinned: true }
						: tab,
			),
		});
	...
	const addPathContext = useCallback(
		(entry: WorkspaceFsEntry): void => {
	...
	const menuForEntry = useCallback(
		(entry: WorkspaceFsEntry): MenuProps => {
			const selectedTarget: LaunchTarget = launchTargets.find(
				(target: LaunchTarget): boolean =>
					target.id === workspaceLaunchTargetId,
			) ??
				launchTargets[0] ?? {
					id: "file-explorer",
					label: "File Explorer",
			...
			const tab: FileTabPreferences = {
				key:
					selectedSourceFolder === null
						? entry.relativePath
						: `${selectedSourceFolder.id}:${entry.relativePath}`,
				sourceFolderId: selectedSourceFolder?.id ?? "",
			...
			return {
				items: [
					...
					{
						key: "open-with",
						label: t("files.menu.openWith"),
						children: launchTargets.map((target: LaunchTarget) => ({
					...
					{
						key: "save-as",
						label: t("files.menu.saveAs"),
						disabled: entry.kind === "folder",
					},
					{ key: "copy-path", label: t("files.menu.copyPath") },
					{
						key: "copy-relative",
						label: t("files.menu.copyRelativePath"),
					},
					{ key: "context", label: t("files.menu.addContext") },
				],
				onClick: ({ key }): void => {
					if (key === "open")
						void openWith(entry, selectedTarget.id).catch(
							(error: unknown): void => {
								void messageApi.error(String(error));
							},
						);
					else if (key.startsWith("open-with:"))
						void openWith(
							entry,
							key.slice(
								"open-with:".length,
							) as WorkspaceLaunchTargetId,
						).catch((error: unknown): void => {
							void messageApi.error(String(error));
						});
					else if (key === "save-as")
						void saveAs(tab, entry).catch(
							(error: unknown): void => {
								void messageApi.error(String(error));
							},
						);
	...
	const titleRender: TreeProps<FileTreeNode>["titleRender"] = useCallback(
		(node: FileTreeNode): React.ReactNode => {
			return (
				<Dropdown
					menu={menuForEntry(node.entry)}
					trigger={["contextMenu"]}
				>
					<span
						className={styles.treeTitle}
						onDoubleClick={(): void => openEntry(node.entry, true)}
					>
						{node.entry.name}
					</span>
				</Dropdown>
	...
	const loadData = useCallback(
		async (node: FileTreeNode): Promise<void> => {
			if (selectedSourceFolder === null || node.entry.kind !== "folder")
				return;
			const entries: FileTreeNode[] = await loadDirectory(
				selectedSourceFolder,
				node.entry.relativePath,
			);
			setTreeData((current: FileTreeNode[]): FileTreeNode[] =>
				replaceTreeChildren(current, node.key, entries),
			);
		},
		...
		const timer: number = window.setInterval((): void => {
			for (const tab of layout.tabs) {
				...
				void window.electronAPI.workspaceFs
					...
					.then((revision): void => {
						if (
							revision.sha256 === buffer.sha256 &&
							revision.modifiedAtMs === buffer.modifiedAtMs
						)
							return;
		...
		return (
			<div className={styles.empty}>
				<Empty
					...
					description={t("files.noWorkspace")}
				/>
			</div>
		);
	}

	const activeSourceFolder: WorkspaceSourceFolder | null =
		activeTab === null
			? selectedSourceFolder
			: (sourceFolders.find(
					(folder: WorkspaceSourceFolder): boolean =>
						folder.id === activeTab.sourceFolderId,
				) ?? selectedSourceFolder);
		...
		activeTab === null
			? [
					{ title: workspace.name },
					{
						title:
							selectedSourceFolder === null
								? t("files.noSourceFolder")
								: getSourceFolderLabel(selectedSourceFolder),
					},
				]
			: [
					{ title: workspace.name },
					{
						title:
							activeSourceFolder === null
								? t("files.noSourceFolder")
								: getSourceFolderLabel(activeSourceFolder),
					},
					...normalizeRelativePath(activeTab.relativePath)
						.split("/")
						.map((part: string) => ({ title: part })),
				];
		...
		<div className={styles.panel} data-studio-files-panel="true">
			...
			<Tabs
				...
				onChange={(key: string): void =>
					patchLayout({ activeTabKey: key })
				}
				onEdit={(targetKey): void => {
					const tab: FileTabPreferences | undefined =
						layout.tabs.find(
							(candidate: FileTabPreferences): boolean =>
								candidate.key === String(targetKey),
						);
			...
			<div className={styles.body}>
				...
				>
					<Splitter.Panel
						min={`${FILE_PANEL_MIN_EDITOR_SPLIT}%`}
						size={
							layout.sidebarOpen ? `${editorSplitSize}%` : "100%"
						}
					>
						<div className={styles.editorPane}>
							{activeTab === null ? (
								<Empty
									image={Empty.PRESENTED_IMAGE_SIMPLE}
									description={t("files.noFileSelected")}
								/>
							) : activeBuffer?.loading ? (
								<Typography.Text type="secondary">
									{t("files.loading")}
								</Typography.Text>
							) : activeBuffer?.error !== null &&
							  activeBuffer?.error !== undefined ? (
								<Alert
									type="error"
									showIcon
									message={activeBuffer.error}
								/>
							) : activeBuffer?.mediaUrl !== undefined &&
							  activeBuffer.mediaKind !== undefined &&
							  activeBuffer.mediaMimeType !== undefined ? (
								<FileMediaPreview
								...
								<>
									...
									<MonacoFileEditor
										...
										tabKeys={layout.tabs.map(
											(tab: FileTabPreferences): string =>
												tab.key,
										)}
										panelKey={panelKey}
										workspace={workspace}
					...
					{layout.sidebarOpen ? (
						<Splitter.Panel
							min={`${100 - FILE_PANEL_MAX_EDITOR_SPLIT}%`}
							size={`${100 - editorSplitSize}%`}
						>
								...
								<div className={styles.sidebarControls}>
									<div className={styles.sidebarPadding}>
										<Select
											...
											options={sourceFolders.map(
												(
													folder: WorkspaceSourceFolder,
												) => ({
													value: folder.id,
													label: getSourceFolderLabel(
														folder,
													),
												}),
											)}
								...
								<div className={styles.treeViewport}>
									<Tree<FileTreeNode>
										...
										onExpand={(keys): void => {
											...
											if (selectedSourceFolder !== null)
												scheduleExpandedPathsPersist(
													selectedSourceFolder.id,
													keys,
												);
										}}
										onSelect={(_keys, info): void => {
											if (info.node.entry.kind === "file")
												openEntry(
													info.node.entry,
													false,
												);
			...
			<Modal
				...
				title={t("files.closeTitle")}
				closable={false}
				mask={{ closable: false }}
				footer={[
					<Button
						key="cancel"
						onClick={(): void => setPendingClose(null)}
					>
						{t("files.cancel")}
					</Button>,
					<Button
						key="discard"
						onClick={(): void => {
							const tab = pendingClose?.tab;
							setPendingClose(null);
							if (tab !== undefined) {
								const buffer = buffers[tab.key];
								if (buffer !== undefined) {
									const next: FileBuffer = {
										...buffer,
										content:
			...
			>
				{pendingClose?.dirty
					? t("files.closeDirty", {
							name: pendingClose.tab.relativePath,
						})
					: null}
			</Modal>
		</div>
