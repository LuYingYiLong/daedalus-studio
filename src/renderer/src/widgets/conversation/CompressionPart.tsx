...
export type TimelineCompressionPart = Extract<TimelineBodyPart, { type: "compression" }>;
...
function CompressionPart({ part, disclosureKey }: CompressionPartProps): React.JSX.Element {
	...
	const details: string = part.status === "completed" ? part.summary : part.reason;
	...
	return (
		<Collapse
			...
			onChange={(keys: string | string[]): void => {
				if (!canExpand) return;
				setOpen((Array.isArray(keys) ? keys : [keys]).includes(itemKey));
			}}
			className={`${styles.collapse} ${canExpand ? "" : styles.collapseStatic}`}
			expandIcon={(): React.JSX.Element => (
				<span className={`${styles.icon} ${active ? styles.iconActive : ""}`} aria-label={typeof label === "string" ? label : t("chat.compression.running")}>
					<Icon name="compress" />
				</span>
			)}
			items={[{
				...
				children: part.status === "completed" ? (
					<div className={`${styles.summary} markdown-body`}><MarkdownContent>{details}</MarkdownContent></div>
				) : (
					<Typography.Text type="secondary" className={styles.reason}>{details}</Typography.Text>
				)
			}]}
		/>
	);
