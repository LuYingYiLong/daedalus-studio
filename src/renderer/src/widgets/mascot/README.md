...
`NewSessionHome` 显示默认尺寸的空闲角色；会话中的 `HomeChatSurface` 将紧凑版角色绝对定位在 composer 右上方，不占用布局高度，鼠标事件穿透
...
- `completed`：胶囊双眼各转两周（1.2s），星环收拢成行星（0.9s），行星绕一圈归位（2.4s）；完成动作仅播放一次，减少动态效果时直接显示归位造型
