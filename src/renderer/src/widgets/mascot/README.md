...
- `sleeping`：空闲三分钟后进入休眠，双眼变为水平眯眼并随呼吸轻微缩放，行星在主星旁缓慢摆动；鼠标、键盘或窗口重新活动时唤醒
- `disconnected`：Backend 连接断开时主星变暗，星环变为断续轨道，行星停滞在脱离位置；重连后恢复实时状态
- `completed`：主星球形轮廓保持静止，贴在球面的胶囊双眼以 CSS 3D 透视绕 Y 轴旋转两周（1.2s）且背面隐藏，星环收拢成行星（0.9s），行星绕一圈归位（2.4s）；完成动作仅播放一次，结束后自动进入 idle；减少动态效果时直接进入 idle
