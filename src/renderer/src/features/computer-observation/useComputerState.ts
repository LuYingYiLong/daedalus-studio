...
export function useComputerDeveloperMode(): boolean {
  ...
  useEffect(() => {
    ...
    const changed = (event: Event): void =>
      setEnabled((event as CustomEvent<GeneralSettings>).detail.developerMode);
    window.addEventListener(GENERAL_SETTINGS_CHANGED_EVENT, changed);
    const dispose = window.electronAPI?.generalSettings?.onChanged((value) => {
      if (active) setEnabled(value.developerMode);
    });
    // 只读挂载不能广播设置变化，否则会重置正在查看的轨迹选中项
    void createBackendClient()
      .then((client) => client.request<GeneralSettings>("generalSettings.get"))
      .then((value) => {
        if (active) setEnabled(value.developerMode);
      })
      .catch(() => {});
