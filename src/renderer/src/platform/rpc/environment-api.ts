...
export async function getEnvironmentConfig(
	...
	return (await createBackendClient()).request("environment.config.get", {
		workspaceId,
		sourceFolderId,
	});
}

export async function updateEnvironmentConfig(params: {
	workspaceId: string;
	sourceFolderId: string;
	content: string;
...
export async function updateEnvironmentTrust(params: {
	workspaceId: string;
	sourceFolderId: string;
	fingerprint: string;
	status: Exclude<EnvironmentTrustStatus, "review-required">;
}): Promise<LocalEnvironmentConfigDocument> {
	return (await createBackendClient()).request(
...
export async function listEnvironmentActions(params: {
	workspaceId: string;
	sourceFolderId: string;
...
export async function listWorktreeStatuses(): Promise<{
	sessions: Array<{
		session: { id: string; title: string; worktree?: unknown };
...
export async function repairWorktree(
	sessionId: string,
): Promise<WorktreeHealthSnapshot> {
	return (await createBackendClient()).request("workspace.worktree.repair", {
		sessionId,
	});
}

export async function createPermanentWorktree(params: {
	workspaceId: string;
	name: string;
	sources?: Record<
		string,
...
export async function deletePermanentWorktree(
	workspaceId: string,
): Promise<{ deleted: true; workspaceId: string }> {
	return (await createBackendClient()).request(
