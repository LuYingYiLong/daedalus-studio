...
import { DaedalusConceptPlayer } from "./concept-video/DaedalusConceptPlayer";
...
function isDocumentationConfigured(
	state: GodotDocumentationState | null,
): boolean {
	return (
		state?.enabled === true &&
		state.documents.some(
			(document: GodotDocumentationRecord): boolean =>
				document.health.status === "ready",
		)
	);
}
...
function hasConfiguredGodotProject(
	result: GodotProjectScanResult | null,
): boolean {
...
function ProviderOnboardingStep({
	selection,
	onSelectionChange,
	onBusyChange,
	...
	>([]);
	const [testedProviderId, setTestedProviderId] = useState<string | null>(
		null,
	);
	...
	const provider: ProviderModelSelectionProvider | null =
		useMemo((): ProviderModelSelectionProvider | null => {
			return (
				selection.providers.find(
					(item: ProviderModelSelectionProvider): boolean =>
	...
	const availableModels: Array<ProviderModelInfo | DiscoveredProviderModel> =
	...
	const needsConnectionTest: boolean =
		provider !== null && (!provider.configured || credentialsChanged);
	...
	async function testConnection(): Promise<void> {
		...
		try {
			...
			if (discovery.source !== "api" || discovery.error !== undefined) {
				throw new Error(
					discovery.error ?? t("onboarding.provider.errors.test"),
				);
			}
			...
			setError(
				getErrorMessage(
					connectionError,
					t("onboarding.provider.errors.test"),
				),
			);
		} finally {
			setTesting(false);
	...
	async function saveAndActivate(): Promise<void> {
		...
		setError(null);
		try {
			const discoveredModel: DiscoveredProviderModel | undefined =
			...
			setError(
				getErrorMessage(
					saveError,
					t("onboarding.provider.errors.save"),
				),
			);
		} finally {
			setSaving(false);
	...
	return (
		<div className={styles.stepContent}>
			...
			{isProviderConfigured(selection) ? (
				<Alert
					showIcon
					type="success"
					title={t("onboarding.provider.existingReady")}
				/>
			) : null}
			{error !== null ? (
			...
			<Form layout="vertical" className={styles.form}>
				<Form.Item label={t("onboarding.provider.fields.provider")}>
					<Select
						...
						options={selection.providers.map(
							(item: ProviderModelSelectionProvider) => ({
								value: item.provider,
				...
				<Form.Item label={t("onboarding.provider.fields.apiKey")}>
					<Input.Password
						...
						placeholder={
							...
							t("onboarding.provider.fields.apiKeyPlaceholder")
						}
						onChange={(event): void => {
				...
				<Flex align="end" gap="small" wrap>
					<Form.Item
						label={t("onboarding.provider.fields.model")}
						className={styles.growingField}
					>
						<Select
							showSearch={true}
							value={modelId}
							placeholder={t(
								"onboarding.provider.fields.modelPlaceholder",
							)}
...
function GodotExecutableOnboardingStep({
	settings,
	onSettingsChange,
}: GodotExecutableStepProps): React.JSX.Element {
	const { t } = useTranslation();
	...
	async function chooseExecutable(): Promise<void> {
		...
		try {
			...
			if (path === null) return;
			const nextSettings: GeneralSettings = await updateGeneralSettings({
				godotExecutablePath: path,
			});
			onSettingsChange(nextSettings);
			if (nextSettings.godotExecutableStatus !== "ready") {
				setError(
					nextSettings.godotExecutableError ??
						t("onboarding.godotExecutable.errors.invalid"),
				);
			}
			...
			setError(
				getErrorMessage(
					chooseError,
					t("onboarding.godotExecutable.errors.select"),
				),
			);
		} finally {
			setLoading(false);
	...
	return (
		<div className={styles.stepContent}>
			<div className={styles.stepHeading}>
				...
				<div>
					<Typography.Title level={2}>
						{t("onboarding.godotExecutable.title")}
					</Typography.Title>
					<Typography.Paragraph type="secondary">
						{t("onboarding.godotExecutable.description")}
					</Typography.Paragraph>
				</div>
			</div>
			{settings.godotExecutableStatus === "ready" ? (
				<Alert
					showIcon
					type="success"
					title={t("onboarding.godotExecutable.ready", {
						version: settings.godotExecutableVersion,
					})}
				/>
			) : null}
			{error !== null ? (
			...
			<div className={styles.settingRow}>
				<div className={styles.settingDescription}>
					<Typography.Text strong>
						{t("onboarding.godotExecutable.path")}
					</Typography.Text>
					<Typography.Text
						type="secondary"
						ellipsis={{
							tooltip: settings.godotExecutablePath ?? undefined,
						}}
					>
						{settings.godotExecutablePath ??
							t("onboarding.godotExecutable.notSelected")}
					</Typography.Text>
				</div>
				<Button
...
function DocumentationOnboardingStep({
	godotVersion,
	onConfiguredChange,
	onBusyChange,
}: DocumentationStepProps): React.JSX.Element {
	const { t } = useTranslation();
	const [documentation, setDocumentation] =
		useState<GodotDocumentationState | null>(null);
	const [branches, setBranches] = useState<GodotDocumentationBranch[]>([]);
	const [recommendedBranch, setRecommendedBranch] = useState<string | null>(
		null,
	);
	...
	const loadDocumentation =
		useCallback(async (): Promise<GodotDocumentationState> => {
			const nextState: GodotDocumentationState =
				await fetchGodotDocumentation();
	...
	useEffect((): (() => void) => {
		...
		void Promise.all([
			loadDocumentation(),
			fetchGodotDocumentationBranches(false),
		])
			...
			.catch((loadError: unknown): void => {
				console.error(
					"[Onboarding] documentation bootstrap failed",
					loadError,
				);
				if (!cancelled)
					setError(
						getErrorMessage(
							loadError,
							t("onboarding.documentation.errors.load"),
						),
					);
			})
			.finally((): void => {
	...
	useEffect((): void => {
		...
		const versionMatch: string | null =
			godotVersion?.match(/^(\d+\.\d+)/)?.[1] ?? null;
		const selected: GodotDocumentationBranch | undefined =
			branches.find(
				(item): boolean =>
					!item.installed && item.name === versionMatch,
			) ??
			branches.find(
				(item): boolean =>
					!item.installed && item.name === recommendedBranch,
			) ??
	...
	const busy: boolean =
		job !== null && !TERMINAL_DOCUMENTATION_JOB_STAGES.has(job.stage);
	useEffect((): (() => void) => {
		onBusyChange(busy);
		return (): void => onBusyChange(false);
	}, [busy, onBusyChange]);
	useEffect(
		(): void =>
			onConfiguredChange(isDocumentationConfigured(documentation)),
		[documentation, onConfiguredChange],
	);

	useEffect((): (() => void) | void => {
		if (!busy || job === null) return;
		const timer: number = window.setTimeout((): void => {
			void fetchGodotDocumentationJob(job.jobId)
				.then((nextJob: GodotDocumentationJob | null): void => {
					const normalizedJob: GodotDocumentationJob | null =
						nextJob ?? null;
					setJob(normalizedJob);
					if (normalizedJob === null) return;
					if (
						TERMINAL_DOCUMENTATION_JOB_STAGES.has(
							normalizedJob.stage,
						)
					) {
						void loadDocumentation();
						if (normalizedJob.stage === "failed")
							setError(
								normalizedJob.error ?? normalizedJob.message,
							);
					}
				})
				.catch((pollError: unknown): void =>
					setError(
						getErrorMessage(
							pollError,
							t("onboarding.documentation.errors.job"),
						),
					),
				);
		}, 600);
	...
	async function startInstall(): Promise<void> {
		...
		try {
			...
			setError(
				getErrorMessage(
					installError,
					t("onboarding.documentation.errors.install"),
				),
			);
		}
	}
	...
	async function cancelJob(): Promise<void> {
		...
		try {
			...
			setError(
				getErrorMessage(
					cancelError,
					t("onboarding.documentation.errors.cancel"),
				),
			);
		}
	}
	...
	async function toggleEnabled(enabled: boolean): Promise<void> {
		...
		try {
			...
			setError(
				getErrorMessage(
					toggleError,
					t("onboarding.documentation.errors.toggle"),
				),
			);
		} finally {
			setToggling(false);
	...
	const readyDocuments: GodotDocumentationRecord[] =
		documentation?.documents.filter(
			(document: GodotDocumentationRecord): boolean =>
				document.health.status === "ready",
		) ?? [];
	...
	return (
		<div className={styles.stepContent}>
			<div className={styles.stepHeading}>
				...
				<div>
					<Typography.Title level={2}>
						{t("onboarding.documentation.title")}
					</Typography.Title>
					<Typography.Paragraph type="secondary">
						{t("onboarding.documentation.description")}
					</Typography.Paragraph>
				</div>
			</div>
			{isDocumentationConfigured(documentation) ? (
				<Alert
					showIcon
					type="success"
					title={t("onboarding.documentation.ready")}
				/>
			) : null}
			...
			{readyDocuments.length > 0 ? (
				<div className={styles.settingRow}>
					<div className={styles.settingDescription}>
						<Typography.Text strong>
							{t("onboarding.documentation.installed", {
								count: readyDocuments.length,
							})}
						</Typography.Text>
						<Typography.Text type="secondary">
							{readyDocuments
								.map((document) => `Godot ${document.branch}`)
								.join(", ")}
						</Typography.Text>
					</div>
					<Space>
						<Typography.Text>
							{t("onboarding.documentation.enableSearch")}
						</Typography.Text>
						<Switch
			...
			{busy && job !== null ? (
				<div className={styles.progressPanel}>
					<Flex justify="space-between" align="center" gap="small">
						<Typography.Text strong>
							{t(`onboarding.documentation.stages.${job.stage}`)}
						</Typography.Text>
						<Button
							size="small"
							onClick={(): void => {
				...
				<Flex align="end" gap="small" wrap>
					<Form.Item
						label={t("onboarding.documentation.branch")}
						className={styles.growingField}
					>
						<Select
							loading={loading}
							...
							value={branch}
							placeholder={t(
								"onboarding.documentation.selectBranch",
							)}
...
function GodotBridgeOnboardingStep({
	...
	useEffect(
		(): void => onConfiguredChange(hasConfiguredGodotProject(result)),
		[onConfiguredChange, result],
	);
	...
	async function installSelected(): Promise<void> {
		...
		for (const projectId of selectedIds.map(String)) {
			const project: GodotProjectInfo | undefined =
				nextResult.projects.find(
					(item): boolean => item.id === projectId,
				) ??
			...
			try {
				if (
					...
					nextResult = await window.electronAPI.godotProjects.repair(
						project.path,
					);
				} else if (project.status === "disabled") {
					nextResult =
						await window.electronAPI.godotProjects.setEnabled(
							project.path,
							true,
						);
				} else {
					nextResult = await window.electronAPI.godotProjects.install(
						project.path,
					);
				}
	...
	return (
		<div className={styles.stepContentWide}>
			...
			{hasConfiguredGodotProject(result) ? (
				<Alert
					showIcon
					type="success"
			...
			{result?.plugin.errorMessage ? (
				<Alert
					showIcon
					type="error"
					description={result.plugin.errorMessage}
				/>
			) : null}
			{result?.projects.some(
				(project): boolean => project.status === "pending_restart",
			) ? (
				<Alert
					showIcon
					type="warning"
			...
			{error !== null ? (
				<Alert
					...
					type="error"
					description={
						<span className={styles.multilineError}>{error}</span>
					}
				/>
			) : null}
...
function OnboardingWizard({
	...
	const [preferences, setPreferences] = useState<ClientPreferences>(() => ({
		...
		onboarding: isOnboardingPreferences(
	...
	const currentIndex: number = Math.max(
		0,
		ONBOARDING_STEP_IDS.indexOf(currentStep),
	);
	const stepItems = useMemo(
		(): Array<{ title: string }> =>
			ONBOARDING_STEP_IDS.map((stepId: OnboardingStepId) => ({
	...
	const progressPercent: number =
		currentStep === "welcome"
			? 0
			: Math.round(
					(currentIndex / (ONBOARDING_STEP_IDS.length - 1)) * 100,
				);
	...
	function persistCheckpoint(
		...
		void withTimeout(
			...
			ONBOARDING_NAVIGATION_TIMEOUT_MS,
			t("onboarding.errors.timeout"),
		)
			...
			.finally((): void => {
				if (mountedRef.current) {
					setPendingCheckpointCount((count: number): number =>
						Math.max(0, count - 1),
					);
				}
			});
	...
	function goForward(outcome?: OnboardingStepOutcome): void {
		const nextStep: OnboardingStepId =
			ONBOARDING_STEP_IDS[
				Math.min(currentIndex + 1, ONBOARDING_STEP_IDS.length - 1)
			];
		persistStep(nextStep, outcome);
	}

	function goBack(): void {
		const previousStep: OnboardingStepId =
			ONBOARDING_STEP_IDS[Math.max(0, currentIndex - 1)];
	...
	function getSummaryOutcome(
		step: OnboardingConfigurableStepId,
	): OnboardingStepOutcome {
		return configuredByStep[step]
			? "configured"
			: (preferences.onboarding.stepOutcomes[step] ?? "skipped");
	}
	...
	if (currentStep === "welcome") {
		content = (
			<div className={styles.welcome}>
				...
				<DaedalusConceptPlayer
					ariaLabel={t("onboarding.welcome.conceptVideoLabel")}
				/>
			</div>
		);
				...
				extra={
					<div className={styles.summaryRegion}>
						<div className={styles.summaryList}>
							{CONFIGURABLE_STEPS.map(
								(
									stepId: OnboardingConfigurableStepId,
								): React.JSX.Element => {
									const outcome: OnboardingStepOutcome =
										getSummaryOutcome(stepId);
									return (
										<div
											key={stepId}
	...
	const currentConfigurableStep: OnboardingConfigurableStepId | null =
		CONFIGURABLE_STEPS.includes(currentStep as OnboardingConfigurableStepId)
			? (currentStep as OnboardingConfigurableStepId)
			: null;
					...
					<Space>
						...
						{currentStep === "complete" ? (
							...
							>
								{t(
									isEnteringStudio
										? "onboarding.actions.enteringStudio"
										: "onboarding.actions.enterStudio",
								)}
							</Button>
						) : (
							<Button
