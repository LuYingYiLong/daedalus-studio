...
describe("Timeline activity group rendering", () => {
	it("renders backend-provided activity groups without reclassifying timeline events", () => {
		const assistantSource: string = readRepoFile("src", "renderer", "src", "widgets", "conversation", "AssistantBubble.tsx");
		const groupSource: string = readRepoFile("src", "renderer", "src", "widgets", "conversation", "TimelineActivityGroup.tsx");
		...
		expect(groupSource).toContain('import ShinyText from "@/ui/ShinyText";');
	...
	it("keeps the activity summary driven by structured counts", () => {
		const groupSource: string = readRepoFile("src", "renderer", "src", "widgets", "conversation", "TimelineActivityGroup.tsx");
		const dataSource: string = readRepoFile("src", "renderer", "src", "domain", "conversation", "timeline-activity-groups.ts");
