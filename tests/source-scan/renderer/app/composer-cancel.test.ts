...
describe("Composer cancellation source", () => {
	it("can cancel while a new session is being created and blocks the pending chat RPC", () => {
		...
		expect(eventStreamSource).toContain("params.cancelledChatRequestIdsRef.current");
		expect(appSource).toContain("if (cancelledChatRequestIdsRef.current.has(requestId))");
		expect(appSource.indexOf("if (cancelledChatRequestIdsRef.current.has(requestId))"))
			.toBeLessThan(appSource.indexOf("await sendChatMessage({", appSource.indexOf("async function handleHomeComposerSubmit")));
		expect(appSource).toContain("cancelledChatRequestIdsRef.current.delete(requestId);");
