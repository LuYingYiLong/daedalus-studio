...
describe("SettingsItem", () => {
	it("provides the shared title, description, and control layout for settings pages", () => {
		const componentSource: string = readRepoFile("src", "renderer", "src", "ui", "SettingsItem.tsx");
		const cssSource: string = readRepoFile("src", "renderer", "src", "ui", "SettingsItem.module.css");
		const generalPageSource: string = readRepoFile("src", "renderer", "src", "widgets", "settings", "GeneralSettingsPage.tsx");
		const searchPageSource: string = readRepoFile("src", "renderer", "src", "widgets", "settings", "SearchSettingsPage.tsx");
		...
		expect(generalPageSource).toContain('import SettingsItem from "@/ui/SettingsItem";');
		expect(searchPageSource).toContain('import SettingsItem from "@/ui/SettingsItem";');
	});
});
