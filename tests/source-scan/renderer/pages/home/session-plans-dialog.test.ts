...
describe("SessionPlansDialog progressive loading", (): void => {
	...
	it("loads one plan preview on demand and exposes local loading states", (): void => {
		const homePageSource: string = readRepoFile("src", "renderer", "src", "pages", "home", "HomePage.tsx");
		const listDialogSource: string = readRepoFile("src", "renderer", "src", "pages", "home", "SessionPlansDialog.tsx");
		const previewDialogSource: string = readRepoFile("src", "renderer", "src", "pages", "home", "SessionPlanPreviewDialog.tsx");
