...
describe("Home dock fullscreen", () => {
	it("toggles the dock control and uses the chat area as the fullscreen bounds", () => {
		...
		expect(shelfSource).toContain("className={styles.footer}");
		...
		expect(shelfCss).toContain("will-change: transform, opacity;");
		expect(shelfCss).toContain("prefers-reduced-motion");
		...
		expect(composerSource).toContain("preserveWorkspaceFooter");
		expect(composerSource).toContain("shouldRenderWorkspaceFooter");
		expect(homeCss).toContain("grid-template-rows: auto auto minmax(0, 1fr) auto;");
		expect(homeCss).not.toContain("chatBodyCoveringFooter");
		expect(homeCss).not.toContain("grid-row: 3;");
		expect(homeCss).toContain("translate3d(-50%, 100%, 0)");
		expect(composerSource).toContain("coverWorkspaceFooter");
		expect(composerCss).toContain(".composerSurfaceCoversWorkspaceFooter");
		expect(composerCss).toContain("margin-bottom: calc(-1 * var(--composer-workspace-footer-cover-height));");
		expect(homeSource).not.toContain("composerFooterDrawer");
		expect(homeCss).not.toContain("composerFooterDrawer");
